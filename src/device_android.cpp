...
{
    m_android = nullptr;
    m_sensor_manager = nullptr;
    m_sensor_event_queue = nullptr;
    m_close = false;
...
DeviceAndroid::~DeviceAndroid()
{
    ...
    if (m_android != nullptr)
    {
        m_android->userData = nullptr;
        m_android->onAppCmd = nullptr;
        m_android->onInputEvent = nullptr;
    }
}
...
bool DeviceAndroid::initDevice(const CreationParams& creation_params)
{
    ...
    while (!m_is_started || !m_is_focused || m_is_paused)
    {
        ...
        int id = ALooper_pollAll(-1, nullptr, &events, (void**)&source);

        if (id == LOOPER_ID_MAIN && source != nullptr)
        {
            source->process(m_android, source);
        }
    }
    
    if (m_android->window == nullptr)
        return false;
...
bool DeviceAndroid::processEvents()
{
    while (!m_close)
    {
        ...
        int id = ALooper_pollAll(should_run ? 0 : -1, nullptr, &events,
                                 (void**)&source);
...
void DeviceAndroid::initSensors()
{
    ...
    m_sensor_event_queue = ASensorManager_createEventQueue(m_sensor_manager,
                                m_android->looper, LOOPER_ID_USER, nullptr, nullptr);
}

void DeviceAndroid::closeSensors()
{
    if (m_sensor_event_queue != nullptr)
    {
